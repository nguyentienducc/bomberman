...
public class Animation {
   private String name;
    ...
    public void drawResize(int x, int y,int width,Graphics2D g2){
        BufferedImage image=getCurrentImage();
       g2.drawImage(image.getScaledInstance(width, width*image.getHeight()/image.getWidth(),image.SCALE_DEFAULT),x-width/2, y-width*(image.getHeight()/image.getWidth())/2,null);
         if(drawRectFrame)
            g2.drawRect(x-width/2, x-width/2, width, width*(image.getHeight()/image.getWidth()));
    }
